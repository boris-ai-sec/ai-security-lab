...
class TelemetrySession:
    ...
    def finish(self) -> bool:
        ...
        if self.provider is not None:
            export_ok = bool(self.provider.force_flush(timeout_millis=5000))
            self.provider.shutdown()
            if not export_ok:
                self.status = "export_failed"
                self.error_type = "otlp_export_failed"
        self._write_sidecar(export_ok=export_ok)
        return export_ok

    def attributes(self, values: dict[str, Any]) -> dict[str, Any]:
        return {key: value for key, value in values.items() if value is not None}

    def _write_sidecar(self, export_ok: bool | None = None) -> None:
